...
module m_config
  ...
  type CFG_t
    ...
    !
  interface CFG_get_dynamic
    module procedure CFG_get_dynamic_logical
    module procedure CFG_get_dynamic_real
    module procedure CFG_get_dynamic_int
  end interface CFG_get_dynamic
    ...
    do ix = 1, command_argument_count()
       ...
       if (arg(1:1) == '-' .and. arg(2:2) /= '-') then
          ...
          if (.not. valid_syntax) then
            ...
            (extension == ".ini" .or. extension == ".txt")) then
...
999 continue ! If there was an error, the routine will end here
  ...
  subroutine CFG_get_dynamic_logical(cfg,arrayname,array1)
    implicit none
    type(CFG_t) :: cfg
    ...
    call CFG_get_size(cfg,arrayname,asize)
    if(asize.gt.0)then
        allocate(tmp(asize))
        call cfg_get(cfg,arrayname,tmp)
        asize=min(size(array1),asize)
        do i=1,asize
            array1(i)=tmp(i)
        end do
        deallocate(tmp)
    end if
  end subroutine CFG_get_dynamic_logical
  
  subroutine CFG_get_dynamic_real(cfg,arrayname,array1)
    implicit none
    type(CFG_t) :: cfg
    character(*),intent(in)::arrayname
  ...
  end subroutine CFG_get_dynamic_real
  
  subroutine CFG_get_dynamic_int(cfg,arrayname,array1)
    implicit none
    type(CFG_t) :: cfg
    character(*),intent(in)::arrayname
  ...
  end subroutine CFG_get_dynamic_int
